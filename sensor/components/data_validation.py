...
class DataValidation:
    ...
    def drop_missing_values_columns(self, df:pd.DataFrame, report_key_name:str)->Optinal[pd.DataFrame]:
        ...
        ===========================================================================================
        returns pandas Dataframe if atleast one column is available after missing columns drop else None
        ...
        try:
            ...
            logging.info(f"selecting column name which contains null values above threshold {threshold}")
    ...
    def is_required_column_exists(self, base_df:pd.DataFrame, current_df:pd.DataFrame, report_key_name:str)->bool:
        ...
        try:
            ...
            if len(missing_columns)>0:
                self.validdation_error[report_key_name]=missing_columns
                return False
            return True
        except Exception as e:
            raise SensorException(e, sys)
    ...
    def data_drift(self, base_df:pd.DataFrame, current_df:pd.DataFrame, report_key_name:str):
        try:
            ...
            for base_column in base_columns:
                ...
                else: # different distribution
                    drift_report[base_column]={
                        "pvalue":float(same_distribution.pvalue),
                        "same_distribution": False
                    }
            
            self.validdation_error[report_key_name]=drift_report
        except Exception as e:
            raise SensorException(e, sys)
    ...
    def initiate_data_validation(self)->artifact_entity.DataValidationArtifact:
        try:
            ...
            logging.info("Replace na values in base df")
            base_df.replace({"na":np.NAN}, inplace=True)
            logging.info("Drop null value columns from the base df")
            base_df = self.drop_missing_values_columns(df=base_df, report_key_name="missing_values_with_base_dataset")
            ...
            logging.info("Drop null values form train df")
            train_df = self.drop_missing_values_columns(df=train_df, report_key_name="missing_values_within_test_dataset")
            logging.info("Drop null values form test df")
            ...
            logging.info("Is all required columns presert in test df")
            ...
            if test_df_columns_status:
                logging.info("As all columns are available in the train df hence detecting data drift")
                self.data_drift(base_df=base_df, current_df=test_df, report_key_name="data_drift_within_test_dataset")

            # write the report
            logging .info("Writing report in yaml file")
            utils.write_yaml_file(file_path=self.data_validation_config.report_file_path,
            data=self.validdation_error)
