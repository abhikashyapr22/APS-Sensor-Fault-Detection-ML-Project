...
with DAG(
    ...
    start_date = pendulum.datetime(2022, 22, 15, tz="UTC"),
    catchup = False,
    tags = ['example'],
) as dag:
    ...
    def sync_prediction_dir_to_s3_bucket(**kwargs):
        bucket_name = os.getenv("BUCKET_NAME")
        # upload prediction folder to prediction files in s3 bucket
        os.system(f"aws s3 sysc /app/prediction s3://{bucket_name}/prediction_files")
